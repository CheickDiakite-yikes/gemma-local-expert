...
from engine.contracts.api import AssetAnalysisStatus, AssetKind, AssetSummary, TranscriptMessage
...
class OrchestratorService:
    ...
    async def stream_turn(
        ...
        if route.proposed_tool:
            tool_plan = self.tool_runtime.plan(
                turn,
                route.proposed_tool,
                results,
            ...
            yield ConversationStreamEvent(
                ...
                conversation_id=turn.conversation_id,
                turn_id=turn_id,
                payload={"tool_name": route.proposed_tool, "payload": tool_plan.payload},
            )
            if policy.approval_required:
                yield self._status_event(
                    ...
                    detail=f"{route.proposed_tool} needs explicit confirmation before it runs.",
                )
                ...
                yield self._status_event(
                    ...
                    detail=f"Executing {route.proposed_tool} inside the local engine.",
                )
                yield ConversationStreamEvent(
                ...
                tool_result = self.tool_runtime.execute(route.proposed_tool, tool_plan.payload)
                assistant_asset_ids = [
                    asset_id
